...
def generate_crystal_portrait(config):
    ...
    if not os.path.exists(source_image):
        print(f"[Warning] Source image not found at '{source_image}'. Creating placeholder.")
        svg_parts = [
            f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {canvas_size} {canvas_size}" width="100%" height="auto">',
            f'  <rect width="{canvas_size}" height="{canvas_size}" fill="#161b22" rx="24"/>',
            f'  <text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" fill="#8b949e" font-family="monospace" font-size="32">Place source photo at {source_image}</text>',
            f'  <text x="50%" y="58%" dominant-baseline="middle" text-anchor="middle" fill="#8b949e" font-family="monospace" font-size="24">and re-run scripts/generate_portrait.py</text>',
            '</svg>\n'
        ]
